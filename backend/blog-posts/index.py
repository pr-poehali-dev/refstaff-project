...
def generate_article(existing_topics: list, existing_titles: list) -> dict:
    ...
    prompt = f"""Ты SEO-копирайтер для блога HR-платформы {BRAND} ({SITE_URL}).
{BRAND} — сервис реферального рекрутинга с геймификацией, но блог охватывает широкую HR-тематику: подбор персонала, адаптация, удержание, HR-аналитика, тренды рынка труда.
...
- В конце: краткий итог (2-3 предложения) + нативное упоминание {BRAND} как инструмента (только если тема близка к рекрутингу/реферальному найму)
